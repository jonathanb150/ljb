...
def getStockDescription(table_name):
    conn = dbConnection()
    cursor = conn.cursor()
    query = "SELECT name from `items` WHERE symbol='%s'" %table_name
    cursor.execute(query)
    row = cursor.fetchone()
    description = row[0]
...
##------------------------------------------------------------------------------
## Analyze currency pairs and sort them
    currencypairs_names = showAllTableNamesAssets('currency')

# Change in the last 3,1 year and 6,3 months
    sorted_data,sorted_tables = priceChangeSort(currencypairs_names,[0.25,0.5,1,3])
    
    descriptions = []
    currencypairs_symbols = []
    for i in sorted_tables:
        currencypairs_symbols.append(i[:-3])
    for i in currencypairs_names:
