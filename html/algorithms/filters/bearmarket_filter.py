#!/usr/bin/env python
#FILE NAME: bearmarket_filter.py
import sys, os
...
def main():
    ...
    for i in stock_symbols:
        ...
        try:
            previous_price = (stock.all_data['closing_price'][:]).max()
            today = (stock.all_data['closing_price'][-40:]).min() # min in the last 2 months
            change = (today - previous_price)/previous_price *100
            if today*1.4<previous_price:
