...
#filename: statistical_backtesting.py
import sys
sys.path.append("/var/www/ljb.solutions/html/algorithms/lib")
from statisticalAnalysisPackage import historical_changes,list_todf,statistics,getData_fromdb
import numpy as np
import pandas as pd
import math
...
def process_data(all_data):
    table = ['Daily POS AVG Change', 'Daily NEG AVG Change', 'Weekly POS AVG Change','Weekly NEG AVG Change', 
    ...
    table2_header = ['Daily POS AVG Change', 'Daily POS AVG Std', 'Daily NEG AVG Change', 'Daily NEG AVG Std',
    'Weekly POS AVG Change', 'Weekly POS AVG Std','Weekly NEG AVG Change', 'Weekly NEG AVG Std',
    'Monthly POS AVG Change', 'Monthly POS AVG Std','Monthly NEG AVG Change', 'Monthly NEG AVG Std',
    'Yearly POS AVG Change','Yearly POS AVG Change', 'Yearly NEG AVG Change','Yearly NEG AVG Change',
    'POS/NEG Days']
    table2 = []
    results = []
    i = 0
...
def clean_outliers(all_data,price_data):
    ...
    table2_header =[['Daily POS AVG Change', 'Daily POS Variance', 'Daily NEG AVG Change', 'Daily NEG Variance',
    'Monthly POS AVG Change', 'Monthly POS Variance','Monthly NEG AVG Change', 'Monthly NEG Variance','Last 30 POS/NEG Days'],[]]
...
def main():
    ...
    try:
        if len(sys.argv[3])!=0:
            initdate = sys.argv[3]
            dt = datetime.timedelta(days=375*1)
    except:
        dt = datetime.timedelta(days=375*3)

    initdate = datetime.datetime.strptime(finaldate,'%Y-%m-%d') - dt
    initdate = datetime.datetime.strftime(initdate,'%Y-%m-%d')

# list with daily,weekly,monthly,yearly changes
    all_data = historical_changes(stock_symbol,initdate,finaldate)
