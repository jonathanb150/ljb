...
## Getting more changes to plot and display info
    ...
    cash_flow = str((int(stock_f.cash[0])-int(stock_f.cash[4]))/1000000)+'M'
    current_cash = str(int(stock_f.cash[0])/1000000)+'M'
    marketcap = str(round(int(stock_f.marketcapquarterly[0])/1000000000,2))+'B'
    dividendyield = str(round(float(stock_f.dividendyieldquarterly[0]),2))+'%'
##Plotting
    makingPlots(stock_f)
...
## Making Json
    ...
    header3 = ['True Price','Expected Price','Profit Margin','Safety','Cash Flow (1Y)','Current Cash','Market Cap','Dividend Yield']
    dic['tablepe1'] = [header3,[str(round(float(trueprice1),2))+'$',str(round(float(expectedprice1),2))+'$',profit_margin,float(points),cash_flow, current_cash, marketcap,dividendyield]]
    dic['portfolio'] = [expectedprice1]
    if pe2!=0 or pe2!=0.0:
        dic['tablepe2'] = [header3,[str(round(float(trueprice2),2))+'$',str(round(float(expectedprice2),2))+'$',profit_margin,float(points),cash_flow, current_cash, marketcap,dividendyield]]
...
def barPlots(info,dates,names):
    ...
    plotly.offline.plot(fig, filename='/var/www/ljb.solutions/html/graphs/plot1FundamentalsChange.html')
...
def currentChanges(changes,index=0,distance=4,period=1):
        change = []
        for i in changes:
            if len(i)<(2*distance+index-1):
                temp_changes = 0
            else:
                temp_changes = [round(i[index],2),round(i[distance+index-1],2),round(i[2*distance+index-1],2)]
...
def historicalValues(stock_f,dates,changes): 
    ...
    for counter in range(len(changes[0])-8):
        ...
        true_values.append(round(float(stock_f.true_value),2))
        expected_values.append(round(float(stock_f.expected_value),2))
    ...
    for i in values_list:
        ...
        for x in temp:
            if x>0 and x<2.5:
                ...
                change = round(change*1.9,2) # 1.9 is cap of growth

        final = (((change**(1/len(i))-1)*100))

        final_changes.append(round(final,2))
...
def getFinancialsChange(financials,period):    
    ...
    for i in financials:
        ...
        for w in range(length):
            if i.iloc[w]==0 or i.iloc[w+period]==0:
                change = 0
            else:
                change = round(((i.iloc[w]-i.iloc[w+period])/i.iloc[w+period])*100,2)
...
def makeTable(dates,true_values,expected_values,price_stats,current_prices):
    ...
    for i in dates:
        ...
        temp = [i,round(current_prices[c],2),round(true_values[c],2),round(expected_values[c],2),price1+' - '+ price2,price_stats[c][2],price_stats[c][3]]
        table.append(temp)
        c+=1
    return table[:]
