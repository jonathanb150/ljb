...
# setting init date selected by user
 ...
 ## ------------------------ PLOTTING ------------------------------------------------------------  
    ...
    interestRates = stockObject()
    interestRates.stock_name ='FEDFUNDS'
    interestRates.init_date = sys.argv[2]
    interestRates.final_date = sys.argv[3]
    interestRates.getData_df()
    interestRates.requested_data = interestRates.all_data 
    # Finding Interest Rate Change - Pre Recession/Crash
    interestRates.analyzingInterestRates()
## ------------------------ PLOTTING ----------------------------------------- 
    makingPlot(stock,interestRates,stoch,d,st_stars_index)
    #output
    info_json = phpOutput(stock,d)
    print(info_json)
    
def makingPlot(stock,interestRates,stoch,d,st_stars_index):
    ...
    ir_dates2 = interestRates.all_data.index.values
    ir_price = high[ir_dates]
    ir_price2 = high[ir_dates2]
    ...
    trace2 = go.Scatter(x = st_stars , y = closing[st_stars_index],
    ...
    trace3 = go.Scatter(x = ir_dates , y = ir_price,
                        ...
                        name = 'Interest Rates Change', yaxis = 'y1')
    ...
    trace6 = go.Scatter(x = dates , y = volume/max(volume)*ratio+ratio2, name = 'Volume', visible = False
                        ,yaxis ='y1')
    trace7 = go.Scatter(x=dates,y=stock.sma1,name ='SMA200', yaxis ='y1',visible = True)
    trace8 = go.Scatter(x=dates,y=stock.sma2,name ='SMA100', yaxis ='y1',visible = True)
    trace9 = go.Scatter(x=dates,y=stock.sma3,name ='SMA50', yaxis ='y1',visible = True)

    trace10 = go.Scatter(x = ir_dates2 , y = ir_price2,
                        mode = 'markers', marker = dict(symbol = 'star',size=10) ,
                        name = 'Interest Rates Down', yaxis = 'y1')
    
    data = [trace1,trace2,trace3,trace00,trace0,trace4,trace5,trace6,trace7,trace8,trace9,trace10]
    ...
    updatemenus=list([
        dict(
            buttons=list([   
                dict(
                    args=[{'visible': [True,True,True,True,True,False,False,True,True,True,True,True]}],
                    label='Volume',
                    method='update',
                ),
                dict(
                    args=[{'visible': [True,True,True,True,True,True,True,False,True,True,True,True]}],
                    label='Stochastic',
                    method='update',
                    ),
                dict(
                    args=[{'visible': [True,True,True,True,True,False,False,False,True,True,True,True]}],
                    label='Reset',
                    method='update',
                ),
