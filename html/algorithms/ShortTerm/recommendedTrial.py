...
def main():
    ...
    pes = (sys.argv[2]).split(" ")
    counter = 0
    for compname in companies:
        stock_f = financialObject()
        stock = stockObject()
	        #inputs
        stock_f.name = compname + '_f' # str company symbol
        stock.stock_name = compname + '_1d'# str company symbol
        pe1 = float(pes[counter])
...
def financialsLast3Years(stock_f,financial_df):
    ...
    while counter!=50000: # 50k so It doesnt run forever in case of error
        ...
        if year == finalyear:
            ...
            if abs(temp_month-desired_month)<=3:
                financial_df = financial_df[counter:counter+12+4]# 12 = 3 years in quarters, +4 = an extra year to find change in last 3 years
                break
        counter+= 1
