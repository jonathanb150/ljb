...
<form class="analyze" onsubmit="return analyzeForm();" method="POST">
	<h1><i class="fas fa-search"></i>Search Indexes</h1>
...
<script type="text/javascript">
	itemSearch("index", $("input[name='item_search']"), $("#search_results"));
	//FORM SUBMIT
	function analyzeForm() {
		if ($("input[name='selected_item']").length == 1) {
			$(".analyze").attr("action", "/analyze_indexes.php?item="+$("input[name='selected_item']").val()+"#analysis_results");
